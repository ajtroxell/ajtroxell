...
var paths = {
  ...
  views: {
    src: '../../views/**/',
    destScript: 'assets/src/js/vendor/',
    destStyles: 'assets/src/scss/vendor/'
  }
};
...
var files = {
  ...
  scriptsMain: [
    ...
    paths.scripts.src + 'vendor/views.js',
...
gulp.task('viewstyles', function () {
  return gulp.src(paths.views.src + '*.css')
    .pipe(concat('views.css'))
    .pipe(wait(500))
    .pipe(sass().on('error', sass.logError))
    .on('error', notify.onError({
      message: 'Views CSS compiling failed'
    }))
    .pipe(postcss(processors))
    .pipe(cleanCSS())
    .pipe(rename({ extname: '.scss' }))
    .pipe(gulp.dest(paths.views.destStyles))
    .pipe(notify({
...
gulp.task('viewscripts', function () {
  return gulp.src(paths.views.src + '*.js')
    .pipe(concat('views.js'))
    .pipe(wait(500))
    .pipe(uglify().on('error', notify.onError({
      message: 'Views JS failed'
    })))
    .pipe(gulp.dest(paths.views.destScript))
    .pipe(notify({
...
gulp.task('default', ['images'], function () {
  gulp.start('viewstyles');
  gulp.watch([paths.views.src + '**/*.css'], ['viewstyles']);
  gulp.watch(paths.styles.src + '**/*.scss', ['compress-css']);

  gulp.start('viewscripts');
  gulp.watch([paths.views.src + '**/*.js', '!../../views/views.js'], ['viewscripts']);
